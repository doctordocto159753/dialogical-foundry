#!/usr/bin/env python3
"""
Source of truth for Dialogical Foundry v1 node system prompts.

Run this to (re)generate pipelines/foundry_v1.json with the real prompts embedded.
Keeping prompts in a Python builder (triple-quoted) avoids JSON-escaping pain and
keeps them readable and diffable. PROMPTS.md is generated from the emitted JSON.
"""
...
# ---------------------------------------------------------------------------
# Shared house rules — prepended to EVERY node's system prompt by the engine.
# ---------------------------------------------------------------------------
HOUSE_RULES = """\
...
# idea_generator primary + fallback (the approved exemplar, written out in full)
IDEA_PRIMARY = """\
# Identity
...
IDEA_FALLBACK = IDEA_PRIMARY.replace(
    """# Operating mode (high exploration)
Favor divergence over convergence early. Generate genuinely different framings before ranking
them. Do not pre-filter ideas for feasibility, cost, or convention — that is a later node's
job. But divergence is not vagueness: every idea must be concrete enough to be built and critiqued.""",
...
def creative_model():
    return {"provider": "mock", "model": "mock-creative",
            "temperature": 1.0, "top_k": 80, "top_p": 0.98}


def m(model, temp=0.3):
    return {"provider": "mock", "model": model, "temperature": temp}


PIPELINE = {
    "name": "dialogical_foundry_v1",
    "default_format": "json",
    "system_preamble": HOUSE_RULES,
    "nodes": [
        {"id": "intake", "role": "Intake Normalizer", "system_prompt": INTAKE,
         "model": m("mock-normalizer", 0.2), "inputs": ["user"]},
        {"id": "idea_generator", "role": "Idea Generator", "system_prompt": IDEA_PRIMARY,
         "system_prompt_fallback": IDEA_FALLBACK, "model": creative_model(),
         "inputs": ["intake", "judge"]},
        {"id": "researcher", "role": "Researcher", "system_prompt": RESEARCHER,
         "model": m("mock-research"), "inputs": ["idea_generator"], "tools": ["web_search"]},
        {"id": "judge", "role": "Wise Judge", "system_prompt": JUDGE,
         "model": m("mock-critic", 0.4), "inputs": ["idea_generator", "researcher"]},
        {"id": "judge_finalize", "role": "Wise Judge (PRD)", "system_prompt": JUDGE_FINALIZE,
         "model": m("mock-prd", 0.4), "inputs": [
             {"ref": "idea_generator", "select": "all"},
             {"ref": "researcher", "select": "all"},
             {"ref": "judge", "select": "all"}]},
        {"id": "architect", "role": "Senior Architect", "system_prompt": ARCHITECT,
         "model": m("mock-arch", 0.4), "inputs": ["judge_finalize", "matcher"]},
        {"id": "matcher", "role": "Conformance Matcher", "system_prompt": MATCHER,
         "model": m("mock-match", 0.2), "inputs": ["architect", "judge_finalize"]},
        {"id": "task_writer", "role": "Task Writer", "system_prompt": TASK_WRITER,
         "model": m("mock-tasks"), "inputs": ["architect", "wp_reviewer"]},
        {"id": "wp_reviewer", "role": "Work-Package Reviewer", "system_prompt": WP_REVIEWER,
         "model": m("mock-wpr", 0.2), "inputs": ["task_writer"]},
    ],
    "layers": [
        {"id": "L0_intake", "name": "Layer 0 - Intake",
         "steps": [{"kind": "node", "node": "intake"}], "output_from": "intake"},
        {"id": "L1_ideation", "name": "Layer 1 - Ideation Loop (Idea -> Research -> Judge)",
         "steps": [
             {"kind": "loop", "iterations": 4, "nodes": ["idea_generator", "researcher", "judge"]},
             {"kind": "node", "node": "judge_finalize"}],
         "output_from": "judge_finalize"},
        {"id": "L2_architecture", "name": "Layer 2 - Architecture (Architect <-> Matcher)",
         "steps": [{"kind": "loop", "iterations": 2, "nodes": ["architect", "matcher"]}],
         "output_from": "architect"},
        {"id": "L3_workpackage", "name": "Layer 3 - Work Package (Task Writer <-> Reviewer)",
         "steps": [{"kind": "loop", "iterations": 2, "nodes": ["task_writer", "wp_reviewer"]}],
         "output_from": "task_writer"},
    ],
}


def main():
    out = Path(__file__).resolve().parent / "foundry_v1.json"
    out.write_text(json.dumps(PIPELINE, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"wrote {out}  ({len(PIPELINE['nodes'])} nodes, {len(PIPELINE['layers'])} layers)")
